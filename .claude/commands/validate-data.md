...
2. For each file, validate:
   ...
   - **Completeness:** Count NaN/null values per column
   - **Temporal continuity:** No missing quarters (FRED/VIX), no duplicate dates
   - **Range checks:** No negative total assets/liabilities, leverage_ratio between 0.5-10x, percentages between 0-1

3. Load the FRED balance sheet and compute derived metrics using `src.analysis.metrics.compute_derived_metrics()`. Check for:
